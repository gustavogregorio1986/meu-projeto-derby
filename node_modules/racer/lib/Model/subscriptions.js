...
var UnloadEvent = events_1.mutationEvents.UnloadEvent;
var promisify = util.promisify;
Model_1.Model.INITS.push(function (model, options) {
    ...
    model.root._fetchedDocs = new CollectionCounter_1.CollectionCounter();
    // Track the total number of active susbscribes per doc
    model.root._subscribedDocs = new CollectionCounter_1.CollectionCounter();
});
Model_1.Model.prototype.fetch = function () {
    this._forSubscribable(arguments, 'fetch');
    return this;
};
Model_1.Model.prototype.fetchPromised = promisify(Model_1.Model.prototype.fetch);
Model_1.Model.prototype.unfetch = function () {
    this._forSubscribable(arguments, 'unfetch');
    return this;
};
Model_1.Model.prototype.unfetchPromised = promisify(Model_1.Model.prototype.unfetch);
Model_1.Model.prototype.subscribe = function () {
    this._forSubscribable(arguments, 'subscribe');
    return this;
};
Model_1.Model.prototype.subscribePromised = promisify(Model_1.Model.prototype.subscribe);
Model_1.Model.prototype.unsubscribe = function () {
    this._forSubscribable(arguments, 'unsubscribe');
    return this;
};
Model_1.Model.prototype.unsubscribePromised = promisify(Model_1.Model.prototype.unsubscribe);
Model_1.Model.prototype._forSubscribable = function (argumentsObject, method) {
    var args, cb;
    ...
    for (var i = 0; i < args.length; i++) {
        var item = args[i];
        if (item instanceof Query_1.Query) {
            item[method](group());
        }
        else {
...
Model_1.Model.prototype.fetchDoc = function (collectionName, id, cb) {
...
Model_1.Model.prototype.fetchDocPromised = promisify(Model_1.Model.prototype.fetchDoc);
Model_1.Model.prototype.subscribeDoc = function (collectionName, id, cb) {
    cb = this.wrapCallback(cb);
...
Model_1.Model.prototype.subscribeDocPromised = promisify(Model_1.Model.prototype.subscribeDoc);
Model_1.Model.prototype.unfetchDoc = function (collectionName, id, cb) {
    cb = this.wrapCallback(cb);
    this._context.unfetchDoc(collectionName, id);
    // No effect if the document is not currently fetched
    if (!this.root._fetchedDocs.get(collectionName, id))
        return cb();
    var model = this;
...
Model_1.Model.prototype.unfetchDocPromised = promisify(Model_1.Model.prototype.unfetchDoc);
Model_1.Model.prototype.unsubscribeDoc = function (collectionName, id, cb) {
    cb = this.wrapCallback(cb);
    this._context.unsubscribeDoc(collectionName, id);
    // No effect if the document is not currently subscribed
    if (!this.root._subscribedDocs.get(collectionName, id))
        return cb();
    var model = this;
    if (this.root.unloadDelay) {
...
Model_1.Model.prototype.unsubscribeDocPromised = promisify(Model_1.Model.prototype.unsubscribeDoc);
...
Model_1.Model.prototype._maybeUnloadDoc = function (collectionName, id) {
    var model = this;
    ...
    if (this._hasDocReferences(collectionName, id))
        return;
    ...
    if (doc.shareDoc && doc.shareDoc.hasPending()) {
        ...
        if (model.root.collections[collectionName])
...
Model_1.Model.prototype._hasDocReferences = function (collectionName, id) {
